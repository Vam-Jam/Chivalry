...
void onInit(CBlob@ this)
{
	...
	v.max_charge_time = 160;
	v.max_cooldown_time = cooldown_time;

	Vehicle_SetupWeapon(this, v,
	                    cooldown_time, // fire delay (ticks)
	                    5, // fire bullets amount
	                    Vec2f(0,0), // fire position offset
	                    "bomb", // bullet ammo config name
	                    "ram_smash", // bullet config name
	                    "RamSmash", // fire sound
	                    "RamSmash", // empty fire sound
	...
	Vehicle_addWheel(this, v, "WoodenWheels.png", 16, 16, 0, Vec2f(-14.0f, 10.0f));
	...
	if (this.hasAttached() || time < 30 || time_til_fire > 0) //driver, seat or gunner, or just created
	{
		...
		if (getNet().isClient() && delay != 0) //only matters visually on client
		{
			...
			if (arm !is null)
			{
				...
				Vec2f armOffset = Vec2f(-8.0f, -4.0f);
